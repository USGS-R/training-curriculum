...
There are also a number of attributes associated with the data.frame returned - `url`, `queryTime`, `comment`, `siteInfo`, and `RATING`. `RATING` will only be included when `type` is `base`. See the next section for how to access attributes of `dataRetrieval` data frames. **NEED TO LINK SOMETHING HERE**
