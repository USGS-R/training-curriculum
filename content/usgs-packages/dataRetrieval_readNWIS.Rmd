...
```{r findsites, eval=TRUE, echo=FALSE, message=FALSE}
# # How I found the two sites to use for the examples. Commented out to save time when knitting.
# sites_sttmp_fl <- whatNWISsites(stateCd="FL", siteType="ST", parameterCd="00010")
# availdata_fl <- whatNWISdata(siteNumbers=unique(sites_sttmp_fl$site_no))
# sites_fl <- availdata_fl %>% group_by(site_no) %>%
#   summarize(whatdata=paste(unique(data_type_cd), collapse=",")) %>%
#   filter(nchar(whatdata) > 18)
# 
# # groundwater site
# sites_gwtmp_or <- whatNWISsites(stateCd="OR", siteType="GW", parameterCd="00010")
# availdata_or <- whatNWISdata(siteNumbers=unique(sites_gwtmp_or$site_no))
# sites_or <- availdata_or %>% group_by(site_no) %>%
#   summarize(whatdata=paste(unique(data_type_cd), collapse=",")) %>%
#   filter(nchar(whatdata) > 11)
```
...
Abbreviation <- c("POP","TW","PS","SD","L","I","TP","SI","M","C","A")
