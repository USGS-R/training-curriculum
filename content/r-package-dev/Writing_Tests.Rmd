...
```{r ph-setup, include=TRUE, warning=FALSE, message=FALSE}
is.valid.pH <- function(pH){
  pH >= 0 & pH <= 14
}
```

```{r reporter-setup, include=FALSE, warning=FALSE, message=FALSE}
  reporter <- testthat:::find_reporter("list")
  old <- testthat:::set_reporter(reporter)
```
...
```{r testthat-infra, include=TRUE, eval=FALSE}
library(testhat)
library(myPackage)
...
```{r ph-valid, include=TRUE, warning=FALSE, message=FALSE}
library(testthat)
...
```{r ph-invalid, include=TRUE, warning=FALSE, message=FALSE}
...
```{r ph-edges, include=TRUE, warning=FALSE, message=FALSE}
...
```{r test-output, include=FALSE, echo=TRUE, warning=FALSE, message=FALSE}
  reporter$get_results() # only works for ListReporter
```

```{r reporter-cleanup, include=FALSE, warning=FALSE, message=FALSE}
  testthat:::set_reporter(old)
```
...
```{r mock-test, include=TRUE, warning=FALSE, message=FALSE}
