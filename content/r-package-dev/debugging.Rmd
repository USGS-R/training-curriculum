...
<a name="features"></a>
R and Rstudio offer four main techniques for debugging.  You can always insert `print()` statements into your like any other language, but R allows you to be much more sophisticated:
...
To start, lets create some simple functions ourselves.  This example is modified from [Hadley Wickham's page on debugging.](http://adv-r.had.co.nz/Exceptions-Debugging.html#debugging-tools)
`func1` calls `func2`, which calls `func3`, and so on.  We know `func4` will fail since it is adding a numeric and a character, but lets see how we would approach it if we didn't know this.
```{r error=TRUE}
...
Now we can use the same debug buttons or key commands shown above to execute specific lines or blocks of code, or continue.
<img src="../static/img/debugButtons.png" alt="debugging buttons"  title = "debugging buttons">
